...
Constitution: India Code — search "Constitution"
Case Law: Indian Kanoon — download landmark case PDFs
Legislation: PRS India — Bills & Acts summaries
---
...
---
⚡ Performance Tips
Speed up embeddings: Already using `bge-small-en` (lightweight, fast)
Reduce context: Lower `top_k` in `classifier.py` if latency is high
GPU acceleration: If you have a GPU, Ollama uses it automatically
Quantized model: Use `mistral:7b-instruct-q4_K_M` for faster inference
```bash
